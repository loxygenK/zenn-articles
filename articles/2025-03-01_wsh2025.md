---
title: "Web Speed Hackathon 2025 で 10 位を取れたもののレギュ落ちした記録"
emoji: "🏎️"
type: "tech"
topics:
  - wsh
...
結果は、*スコアに関しては* 390/1200 点（全体[^net-score-rank] 10 位）という、悪くはない数値でした!  ……ただし、レギュレーションチェックにおいて、**対象 15 人のうち、14 人がまとめて失格となる** [^regulation-check]という大事件に巻き込まれ、無事順位対象外となりました。南無三。
...
キレイな言葉ではありませんが、神ゲーだったなあと感じています。来年もまた出ます! もしこの記事を読んでくださっている方が出場を迷っていれば、ぜひ出場することをおすすめします!
...
- **余分なリクエストを削りました。**
    - 5000ms タイムアウト付きドデカ preload が入ってたのでやめました。
...
- **デカリソースを解消しました。**
    ...
    - 画像を全部 webp にしました。↑ の変換処理も不要になるのでうれしい
    - そんなに見られないモーダルが巨大なテキストコンテンツを表示していたので、必要に応じてダウンロードするようにしました。
- **余分な依存を解消しました。**
    - three.js をやめました。
    - **AI でファイルタイプを検出する仕組み**が採用されていて、しかも**モデルをローカルに落として、ローカルで処理をしていた**のでやめました。
    - 大きな Web フォントが配信されていた一方で、使われていなかったのでやめました。
- 正規表現が大変なことになっていたので削りました。
...
以降は概ね時系列順で書いていますが、似たような変更に関する記述を一箇所にまとめるために、一部時系列から離れているところがあります。また、変更による細かいデグレの修正など、一部カバーしていないところがあります。
...
テストランから 3 日語、ついに本大会当日を迎えました。

### 初動
時は 10 時 30 分、Web ページを爆速にする 30 時間の火蓋が切って落とされました。

#### デプロイ

WSH2024 では Koyab.app を使ってアプリをデプロイしていたので、今回もそうなのかなあと（何故か）思い込み 事前にデプロイ画面を準備しておいたのですが、**別に Heroku でした**。

Heroku の無料枠が結構前になくなってしまったので、自分の Heroku アカウントでデプロイしようとするとお金がかかってしまいます。そこで、運営様側で準備いただいた Heroku アカウントでデプロイをするための基盤が整えられており、**その仕組みを使えば無料でデプロイができる**という状態になっていました！
これを使えば参加者側での金銭負担はゼロとなるのですが、ログ確認は運営の方に問い合わせ、というデバッグ作業上のオーバーヘッドがあったのと[^not-that-much-overhead]、一つ基盤を挟むことで何かとイシューのもとになるのでは[^deploy-struggle]…… という不安があったため、私は自身の Heroku アカウントでデプロイをすることにしました。
...
先日のテストランで VRT の重要性を学んでいたので、VRT のためのスナップショットを取りました。Arch Linux 上でやっていたせいで最初まともに動かなかったのですが、困りながらとりあえず AUR パッケージ `google-chrome` を入れたところ動いてくれました。

### 動かしてみる

デプロイがちゃんと動き、VRT の準備も整い、とりあえず安心してスタートラインに立つことができたので、実際にアプリケーションの動作の様子を見てみました。

WSH2024 では表示が完了するのを待つ気にもならないほど遅かったのですが、今大会はたしかに遅いものの**別にレンダリングはされており**、少し拍子抜けしました。
...
フォントをファイルに起こそうか迷いましたが、ここ以外でフォントは使われてなさそうだったので、SVG をやめてラスタライズすることにしました。Dock にあった Figma にドラッグアンドドロップしてみると、うまくラスタライズしてくれたので、**Figma に D&D して PNG 化しました**。

JPEG に関しては脳死で Webp に置換しました。ただ、Webp に置換した割には画像が大きく、DevTools で img タグの `src` 属性の値をポイントして、ようやく**画像横幅が 6000px くらいある**ことに気づきました。

![DevTools のスクリーンショット。画像が 6144px あると書いてあります](/images/2025-03-01_wsh2025/image-5.png =500x)

そっちを直さないと結局デカいので、400px、1200px 用の画像も作り、画像がそこまで大きくなければ大きい画像は持ってこないようにしました（普通に 1200px 一つ生やせば足りたかも）。

#### JS をチャンク分けする [(ref)](https://github.com/loxygenK/web-speed-hackathon-2025/commit/aaa42e8e6e89c41d3cedb04b968c9fda3d2ab049)

静的リソースがデカいのはあらかたどうにかなったので、JS がデカいのをどうにかしようとなりました。とにかく FFmpeg がデカすぎるので、将来的には絶対に剥がす決意を固めつつ、とりあえず応急措置として、せめて不要時には FFmpeg が転送されないように設定することにしました。
...
いくつか CSS でいいやんって感じのところを JavaScript でやっていたので、修正しました。
...
*インタラクトしないとわからない仕様に関してモーダルで競技者に周知するの
かしこいかもと思いました。それはそれとしてやめてほしいなんで拡縮するんすか*
...
UnoCSS でスタイルが当たっていたのですが、元のコードベースだと、静的解析 → CSS 生成の形ではなく、ランタイムで className を監視し動的に CSS を生成するという形が取られていました。

最初から CSS が存在するわけではないので、CSS がない瞬間が存在し**最初一瞬スタイルが当たっていない状態でページが表示される**ということが起こっていました。その上、UnoCSS の設定の中にアイコンセットの設定が存在しており、それがバンドルされる結果**例のアイコンセット由来のデカ JSON** がついてくる…… という状態になっていたため、バンドルサイズが大きい原因にもなっていました。

静的解析で上手くいかないこともないだろうと考え、UnoCSS Runtime は剥がし、静的生成するように変更しました。UnoCSS の静的生成は `@unocss/cli` に任せ、`build` 時に追加でコマンドを実行するようにしました。ランタイムに依存している部分（→ 動的解析を行っている部分）は、`style={{}}` に置換しました。
...
バックエンドで SQLite を触るために Drizzle ORM が採用されていたのですが、**何故かそれがバンドルされています**。もちろんブラウザでデータベースを操作することはないのでこれは変です。
...
`assertSchema` の定義と参照、`databaseSchema` の `import` をすべて別ファイルに移し、Drizzle ORM を依存の流れから断ち切りました。今思うと `import * as datbaseSchema from ...` を `import type` にして、関数のシグネチャを調整するだけで足りたかもしれません。
...
ここで Webpack の出力を見てみたのですが、かつて 50 MB くらいあったバンドルサイズが、これまでの作業を経て**719 KiB にまで落ちていました!** いいですね。一般論的にはもっと縮めるべきなのですが、それでも相対的に見ると劇的な変化です（だいたい FFmpeg と UnoCSS 吹っ飛ばして数十 MB が消えたからってだけですが）。
...
また、計測を回したのですが、めちゃくちゃ上振れして 500 点台（全体 2 位）になりました[^sudden-raise-might-be-error]。残念ながらまぐれだったようで、この数値をまた見ることは叶いませんでした。
...
昨年もあったのですが、無効なメールアドレスが入力された場合にめちゃくちゃ重くなる正規表現が使われていました。`..` がメールアドレスのユーザー名部分（`@` より前）に存在して、その周辺の文字数が多いと重くなるみたいです。
...
https://jsbenchmark.com/#

https://jsbenchmark.com/#
...
ここで再度計測を回したところ、**397.35 点** でした！点数こそ向上していますが、順位は上振れ前と変わらず**5 位**と、点数のインフレを感じます。
...
`RecommendedModule` が複数個配信されている…… ということを先述しましたが、各 `RecommendedModule` はそれなりに縦幅を取るので、First view で見えない `RecommendedModule` がいくつか存在します。レンダリングをブロックしてしまう fetch は、First view で本当に必要なものだけに留めたいです。というわけで、**`IntersectionObserver` を使った無限スクロールの形**で、段階的に `RecommendedModule` を取るようにしました[^observing-last-div] [(ref)](https://github.com/loxygenK/web-speed-hackathon-2025/commit/ebf7c7739c15c07f206c03dea5c285d8f1542b5a)。
...
しかし、VRT 下で、**スクロール処理からスクショまでが早く、fetch が間に合わない**という問題が起こってしまいました。無限スクロールに関わるパラメータを微調整すればどうにかならないか、、と思いいろいろ試しましたが、普通に無理だった（し、そういう微調整はよくない）ので、「**ユーザから何らかの操作があったら取る**」という挙動に修正しました[(ref)](https://github.com/loxygenK/web-speed-hackathon-2025/commit/dd0adb5e5c33aa346fe821fb50f11d547a7c0b41)。無事 VRT も動くようになりましたし、初回 fetch のペイロード量も減りました！
...
ここで、**サーバがアメリカにあるの、主に静的コンテンツの配信がキツい！！** と感じ、**Cloudflare をかませてそこを計測対象としました**。この段階ではキャッシュの設定はしていませんでした。
...
何度か計測を行ったのですが、途中で 40 点下振れるという事故がありました。それなりに後悔しましたが、スコアサーバからの「来年度もよろしくお願いします」というメッセージと捉えて耐えました。
...
基本的にスムーズに最適化を進めることができましたが、途中大アクシデントがいくつかありました。

#### いつの間にかめちゃくちゃ落ちるようになってた

1 日目の 14:00 (T+4:00) のことでした。**ここまで来て、VRT がいつの間にか通らなくなっていることに気づきました**。さらに悪いことに、コミットごとに VRT を回すのを忘れており、**どの変更が影響したのかがわかりません**。困りました。
...
とりあえず VRT は動いたのですが、**画像読み込みが途中の段階でレンダリングされる**など、タイミング的な部分で VRT が揺れることが多々ありました。
いろいろためしていると、**`--worker` 数を多く設定している**ということに気づきました。CPU のコア数を指定していたのですが、ワーカーが多すぎても、高負荷環境下のレンダリングとなってしまい結果が揺れてしまうようです[^too-many-workers]。デフォルト値がいい感じの値ということに気づいたので、特に何も指定せず

[^too-many-workers]: ブラウザってシングルスレッドなものじゃないので、それはそう……

#### レンダリングが上手くいかない (気がする)

ヘッダーの部分で、透明度が変わるグラデーションが存在していたのですが、この部分に微妙に差異が発生していました。また、上下位置がズレるという差異も存在していました。
...
- **寝るべき**
  今回の Web Speed Hackathon では**睡眠を取らずに参加した**のですが、**2 日間体調不良を引きずり大後悔しました**。というか当日も最後の方適切な意思決定ができなかったのでパフォーマンスに悪影響を与えています。絶対寝るべきでした。
